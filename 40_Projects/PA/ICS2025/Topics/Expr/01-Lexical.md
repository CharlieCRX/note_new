...
直接将`expr.c`复制到一个测试目录，重命名为`mock_test.c`，然后：
...
int run_test_case(const TestCase *case_) {
  printf("=== 运行测试：%s ===\n", case_->name);
  memset(tokens, 0, sizeof(tokens)); // 清空旧Token，避免跨用例污染
  
  // 步骤1：调用make_token生成Token（核心：独立调用词法分析）
  ...
  if (nr_token != case_->expected_nr_token) {
    printf("❌ 测试失败：Token 数量不匹配！预期：%d，实际：%d\n", case_->expected_nr_token, nr_token);
    return 0;
  }

  // 步骤3：逐Token校验类型 + 字符串（核心校验）
  int pass = 1;
  for (int i = 0; i < nr_token; i++) {
    ...
    // 校验字符串（仅针对需存储字符串的Token）
    if ((tokens[i].type == TK_DEC || tokens[i].type == TK_HEX || tokens[i].type == TK_REG || tokens[i].type == TK_IDENT) && 
        strcmp(tokens[i].str, case_->expected_strs[i]) != 0) {
