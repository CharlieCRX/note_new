...
- 了解表达式的归纳定义（表达式最小边界 + 递归组合）
- `eval(p, q)`的目标是什么？
- Code实现1：检查左右括号是否匹配
  - 思考：面对不合法表达式（例如多一个左括号）和合法表达式但是不属于`"(" <expr> ")"`情况的解决方案
- Code实现2：如何获取表达式的主运算符？（手册已经讲解）

#### 表达式的归纳定义（Recursive Definition）

一个算术表达式可以递归地定义为：

1. 一个**基本表达式**（最小边界`<expr>`）
   - 一个数字
   - 一个寄存器
2. 或者
   - `"(" <expr> ")"`
...
- 语法分析：📄 [`Topics/Expr/02-Parsing.md`](../../Topics/Expr/02-Parsing.md)
